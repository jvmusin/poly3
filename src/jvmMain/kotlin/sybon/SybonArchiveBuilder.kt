...
class SybonArchiveBuilder(
    ...
    companion object {
        const val BUILT_PACKAGES_FOLDER = "ready"
    }

    @Suppress("BlockingMethodInNonBlockingContext")
    suspend fun build(problemId: Int, properties: AdditionalProblemProperties): String {
        try {
    ...
    private suspend fun buildInternal(problemId: Int, properties: AdditionalProblemProperties): String {
        return coroutineScope {
            ...
            suspend fun writeConfig(sampleTests: List<PolygonTest>) {
                val timeLimitMillis = properties.timeLimitMillis ?: problemInfo.await().timeLimit
                val memoryLimitMegabytes = properties.memoryLimitMegabytes ?: problemInfo.await().memoryLimit
                val config = """
                    [info]
                    name = ${statement.name}
            ...
            val zipPath = Paths.get(BUILT_PACKAGES_FOLDER, "$fullName-$uuid.zip")
